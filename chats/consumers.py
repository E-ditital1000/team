...
class ChatConsumer(WebsocketConsumer):
    ...
    def connect(self):
        ...
        # Send online users count when someone connects
        try:
            room = Room.objects.get(id=self.room_id)
            if hasattr(self.scope, "user") and self.scope["user"].is_authenticated:
                room.connect(self.scope["user"])
                self.send_online_users_count(room)
        except Exception as e:
            print(f"Error in connect: {str(e)}")

    def disconnect(self, close_code):
        print("WebSocket closed")
        # Update online users count when someone disconnects
        try:
            room = Room.objects.get(id=self.room_id)
            if hasattr(self.scope, "user") and self.scope["user"].is_authenticated:
                room.disconnect(self.scope["user"])
                self.send_online_users_count(room)
        except Exception as e:
    ...
    def send_online_users_count(self, room):
        # Send updated online users count to the room
        content = {
            "command": "online_users",
            "count": room.connected_clients.count()
        }
